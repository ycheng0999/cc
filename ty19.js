(async () => {
  ...
  function fixKeyIv(str) {
    ...
    if (str.length === 8) return str + str;  // 8字符重复补齐成16
    // 其他情况按需要补齐
  ...
  try {
    ...
    const paths = [
      ["result", "web_url"],        // sd
      ["bio_result_tron", 0, "bio_link_url_tron"], // 另一字段
      ["prd_result_flg", "prd_link_url_flg"]      // 你的第二软件字段
    ];
    ...
    for (const { key, iv } of KEYS) {
      try {
        ...
        $.log(`尝试 key=${key} iv=${iv} 解密失败：${err.message}`);
      }
    }
