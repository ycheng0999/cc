(async () => {
  ...
  try {
    ...
    for (const { key, iv } of KEYS) {
      try {
        ...
        if (decrypted?.trim()) {
          // 判断是否类似域名格式，且不是 vmess 链接
          if (/^[\w.-]+\.[\w.-]+$/.test(decrypted.trim()) && !decrypted.trim().startsWith("vmess://")) {
            const vmessNode = createVmessNode(decrypted.trim());
            const vmessLink = "vmess://" + Buffer.from(JSON.stringify(vmessNode)).toString("base64");
  ...
  function createVmessNode(host) {
    return {
      v: "2",
      ps: "自动转换节点",
      add: host,
      port: "443",
      id: "00000000-0000-0000-0000-000000000000",  // TODO：请替换成你的 UUID
      aid: "0",
      net: "tcp",
      type: "none",
      host: "",
