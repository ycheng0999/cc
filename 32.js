          ...
          try {
            ...
            if (typeof parsed === "object" && parsed.result) {
              ...
              for (const [field, value] of Object.entries(result)) {
                if (typeof value === "string" && isProbablyEncrypted(value)) {
                  for (const { key, iv } of KEYS) {
                    try {
                      const k = CryptoJS.enc.Utf8.parse(key);
                      const i = CryptoJS.enc.Utf8.parse(iv);
                      const fieldDecrypted = AES_Decrypt(decodeURIComponent(value), k, i, CryptoJS);
                      if (fieldDecrypted?.trim()) {
                        decryptedFields[field] = fieldDecrypted;
                        break;
                      }
                    } catch {}
                  }
              ...
              const { address, port, path, node_id } = decryptedFields;
              if (address && port && node_id) {
                ...
                $.msg($.name, "✅ 节点提取成功", vmessLink);
                return $.done({});
              } else {
                $.msg($.name, "✅ 解密 JSON", JSON.stringify(decryptedFields, null, 2));
                return $.done({});
              }
            }
          } catch (e) {
            $.msg($.name, "✅ 文本解密成功", decrypted);
            return $.done({});
          }
        }
      } catch (err) {
