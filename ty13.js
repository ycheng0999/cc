...
(async () => {
  ...
  try {
    ...
    for (const path of PATHS) {
      ...
      const processVal = (v) => {
        if (Array.isArray(v)) {
          ...
          Object.values(v).forEach(processVal);
        } else if (typeof v === "string" && isEncrypted(v)) {
          const dec = tryDecrypt(v);
          if (dec) vmessList.push(dec);
        }
      };
      processVal(val);
    }

    // === 分字段解密，支持 result 是对象或数组 ===
    const candidates = [];
    ...
    // 合并并去重
    const combined = [...new Set([...vmessList, ...candidates])];
    if (combined.length === 0) throw new Error("无法生成任何 vmess 配置\n字段可能未匹配或解密失败");

    combined.forEach(v => $.log(v));
    $.msg("✅ 解密完成", `共生成 ${combined.length} 条配置`, combined[0]);
